...
class SetupPhase extends Phase {
    constructor(game, startingPlayer) {
        super(game, 'setup');
        this.initialise([
            new SimpleStep(game, () => this.setupBegin(startingPlayer)),
            new GameStartPrompt(game),
            new SimpleStep(game, () => this.drawStartingHands()),
            new SimpleStep(game, () => {
                game.getPlayers().forEach(player => {
                    game.emit(`draw-start-hand-${player.name}`, player.getSummaryForCardList(player.hand, player, true));
                });
            }),
    ...
    setupBegin(startingPlayer) {
        let allPlayersShuffled = _.shuffle(this.game.getPlayers());
        this.game.activePlayer = allPlayersShuffled.find(p => p.name === startingPlayer) || allPlayersShuffled.shift();
